...
describe('HomePage', () => {
    ...
    test('should call usePaginatedHero with default values', () => {
        renderHomePage()
        expect(mockUsePaginatedHero).toHaveBeenCalledWith(1, 'all')
    });

    test('should call usePaginatedHero with custom query params', () => {
        renderHomePage(['/?page=2&category=villains'])
        expect(mockUsePaginatedHero).toHaveBeenCalledWith(2, 'villains')
    });

    test('should called usePaginatedHero with default page and same limit on tab', () => {
        renderHomePage(['/?tab=favorites&page=2'])

        const [, , , villainsTab] = screen.getAllByRole('tab')
        fireEvent.click(villainsTab)
        screen.debug()
        expect(mockUsePaginatedHero).toHaveBeenCalledWith(1, 'villain')
    });
